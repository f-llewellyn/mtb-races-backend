		...
		const rawEvents: TRaceRaw[] = await page.evaluate(() => {
			...
			const reducer = (
				...
				if (i % 2 === 0) {
					...
					rows.push(
						...monthRows.map((element) => ({ year, element }))
					);
				}
				return rows;
			};
			...
			return tableItems
				...
				.map(({ year, element }) => ({
					...
					url:
						element
							.querySelector(".eti_title a")
							?.getAttribute("href") ?? null,
					dateText: `${
						element.querySelector(".eti_date")?.textContent
					} ${year}`,
					typeText:
						element
							.querySelector(".etp_cycling")
							?.getAttribute("title") ?? null,
					locationText:
						element
							.querySelector(".eti_map img")
							?.getAttribute("alt") ?? null,
				}));
		});

		const events = rawEvents.flatMap(
